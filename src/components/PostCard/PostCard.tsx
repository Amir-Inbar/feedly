...
export const PostCard: React.FC<PostCardProps> = ({post, onLike}) => {
    ...
    useEffect(() => {
        fetch(`https://backend.tedooo.com/?itemId=${post.id}`).catch(console.error);
    }, [post.id]);
        ...
        >
            <section>
                ...
                <div className="flex justify-between items-center px-[25px] py-4 text-sm text-post-like-count">
                    <div className="flex items-center" onClick={() => onLike(post.id)}>
                        <div
                ...
                <div className="flex justify-evenly items-center p-6">
                    <div className="flex flex-row cursor-pointer " onClick={() => onLike(post.id)}>
                        <FeedlyIcon icon={post.didLike ? 'customLiked' : 'customLike'} className='mr-1'/>
                        <span className={post.didLike ? 'text-post-like-active' : 'text-post-like-inactive'}>
                            Like
                        </span>
                    </div>
